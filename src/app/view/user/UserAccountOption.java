...
public class UserAccountOption extends javax.swing.JInternalFrame {
    ...
    boolean IsValidUser() {
        ...
        Connection conn = DBConn.getConnection();
        String createString;
        createString = "SELECT * FROM user WHERE UserId=" + user_id + " AND Password='" + FunctionFactory.convertMD5(txtcpword.getText()) + "' ";
        ...
        if (isvalid == true) {
            if (npword.isEmpty() == false || cnpword.isEmpty() == false) {
                if (!npword.equals(cnpword)) {
                    ...
                    edit_pword(FunctionFactory.convertMD5(npword), user_id);
                    unameOpt();
                    this.dispose();
